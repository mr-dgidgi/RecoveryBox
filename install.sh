...
menu_services() {
    ...
    read -rp "Enable filebrowser? yes/no (default : yes) : " QuestionEnableFilebrowser
    QuestionEnableFilebrowser=$(yes_no_check "$QuestionEnableFilebrowser")
    if [[ $QuestionEnableFilebrowser -eq 0 ]]; then
        echo -e "$MSGYELLOW" "$SRVMSG" "filebrowser : disabled." "$MSGNC"
...
recoverybox_enable_filebrowser: $EnableFilebrowser
